...
function muestra(mEvento, mWiki){
    ...
    document.getElementById('infoButton').onclick = function(){ cargarContenidoURL('https://us-central1-eventos-98475.cloudfunctions.net/mostrarEventosHtml?evento='); } ;
}
function volver(){
...
function cargarContenidoURL(urlPage){
    $.ajax({
        ...
        $("#wiki").html(data);
        document.getElementById('infoButton').onclick = function(){ muestraEvento(eventoMinuscula) } ;
    })
}